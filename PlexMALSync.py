...
def read_settings(file):
    ...
    if not os.path.isfile(file):
        logger.critical('[CONFIG] Settings file file not found: {}'.format(file))
        sys.exit()
...
def get_anime_shows():
    ...
    logger.info('[PLEX] Retrieving of {} anime shows completed'.format(len(shows)))
    return shows
...
def get_plex_watched_shows(shows):
    ...
    for show in shows:
        ...
        for episode in show.episodes():
            try:
                ...
                if episode.isWatched and n_episode:
                    if (n_episode > episodes_watched and season == season_watched) or \
        ...
        if episodes_watched > 0:
            ...
            logger.info('Watched {} episodes of show: {}'.format(episodes_watched, show.title))
...
def get_mal_list():
    ...
    user = mal_settings['username']
    mal_list = spice.get_list(spice.get_medium('anime'), user, mal_credentials).get_mediums()
...
def match_seasons_on_mal_list(mal_list):
    ...
    # type 1 indicates TV.
    is_tv_show = lambda show: show.raw_data.contents[3].contents[0] == '1'
    show_date = lambda show: show.raw_data.contents[7].contents[0] \
    ...
    for show in tv_shows:
        ...
        try:
            ...
            logger.error('Error during matching season retrieval for show: {}'.format(show.title))
            original_name_treated = show.title

        for i, element in enumerate(matched_list):
            mal_list_seasoned.append((element[0], i+1, original_name_treated))
...
def update_mal_list_with_seasons(mal_list_seasoned, plex_shows):
    ...
    for show, (episodes, season) in plex_shows.items():
        ...
        if bool(matches_in_mal_list_seasoned) or season == 1:
            continue
        mal_shows = spice.search(show, spice.get_medium('anime'), mal_credentials)
        matched_list = []
        for mal_show in mal_shows:
            try:
                ...
                logger.error('Error during season date lookup for show: {}'.format(mal_show))
        matched_list.sort(key=lambda x: x[1])
        ...
        try:
            ...
            logger.error('Error during original name treatment for show: {}'.format(show.title))
            original_name_treated = show.title

        for i, element in enumerate(matched_list):
            mal_list_seasoned_updated.append((element[0], i + 1, original_name_treated, 'not_on_mal_list'))
...
def update_mal_entry(list_item, plex_title, plex_watched_episode_count, force_update):
    ...
    if mal_show_id > 0:
        if mal_watched_episode_count < plex_watched_episode_count or force_update:
            ...
            logger.warning('[PLEX -> MAL] Watch count for %s on Plex is {} and MAL is {}, updating MAL watch count to {} and status to {} ]'
                           .format(plex_title, plex_watched_episode_count,
            ...
            logger.warning('[PLEX -> MAL] Watch count for {} on Plex was equal or higher on MAL so skipping update'
                           .format(plex_title))
        pass
    ...
    for show, value in plex_watched_shows.items():
        ...
        for list_item in mal_list:
            ...
            if mal_title.lower() == plex_title.lower() or mal_title_english.lower() == plex_title.lower():
                ...
                logger.debug('{} [{}] was already in list => status = {} | watch count = {}'
        ...
        if not show_in_mal_list:
            ...
            if not found_result:
                logger.error('[PLEX -> MAL] Failed to find {} on MAL'.format(plex_title))
...
def start():
    ...
    updated_mal_list = update_mal_list_with_seasons(mal_list_seasoned, watched_shows)
